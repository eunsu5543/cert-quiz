...
- **skill-050** [too-deep] single
  - Q: NHN Container Service(NCS)에서 템플릿에 정의하는 컨테이너 유형 중 '일반' 컨테이너와 '초기화' 컨테이너의 차이에 대한 설명으로 맞는 것은 무엇인가요?
  - 사유: NCS 일반/초기화(init) 컨테이너 차이 — K8s 특화 개념으로 CES 입문 깊이 초과(일관성 정리)
